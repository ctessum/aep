...
!
      ...
      SUBROUTINE RUN_MEGCAN(SDATE, STIME, MXREC, NCOLS, NROWS, TSTEP,
...
! variables from input file
      ...
      REAL :: LAT( NCOLS, NROWS )     ! Latitude of grid cell
      REAL :: LONG( NCOLS, NROWS )    ! Longitude of grid cell

      REAL :: LAIc( MXREC, NCOLS, NROWS )    ! Current step LAI
      REAL :: TEMP( MXREC, NCOLS, NROWS )    ! Temperature (K)
      REAL :: PPFD( MXREC, NCOLS, NROWS )    ! Calculated PAR (umol/m2.s)

      REAL :: WIND( MXREC, NCOLS, NROWS )
      REAL :: PRES( MXREC, NCOLS, NROWS )
      REAL :: QV( MXREC, NCOLS, NROWS )

      REAL :: CTF( NRTYP, NCOLS, NROWS ) ! Canopy type factor array
...
! local variables and their descriptions:
      ...
      INTEGER :: Day
      REAL :: Sinbeta,Beta
      REAL,DIMENSION(Layers) :: VPgausDis, QdAbsV, QsAbsV, QdAbsn,     
...
!          CALL M3MESG( MESG )
          ...
          DO I=1, NCOLS
           DO J=1, NROWS
            ...
            IF (TotalCT .GT. 0.0 .AND. LAIc(T,I,J) .GT. 0.0) THEN
...
!               Solar      = PPFD(I,J)/ConvertWm2toUmolm2s*2
                ...
                Call CanopyRad(VPgausDis, Layers, LAIc(T,I,J), Sinbeta, Qbeamv,
     ...
     &               NrCha,NrTyp)
                 ...
                 HumidairPa0  =  WaterVapPres(QV(T,I,J), PRES(T,I,J), WaterAirRatio)
...
! Saturation vapor pressure (millibars)
     ...
     &                      SunFrac) BIND(C,name='run_megcan_c')
      ...
      real(c_float), intent(in)  :: LONG(MXREC, NCOLS, NROWS)
      real(c_float), intent(in)  :: LAIc(MXREC, NCOLS, NROWS)
      real(c_float), intent(in)  :: TEMP(MXREC, NCOLS, NROWS)     
      real(c_float), intent(in)  :: PPFD(MXREC, NCOLS, NROWS)
      real(c_float), intent(in)  :: WIND(MXREC, NCOLS, NROWS)
      real(c_float), intent(in)  :: PRES(MXREC, NCOLS, NROWS)
      real(c_float), intent(in)  :: QV(MXREC, NCOLS, NROWS)     
