...
while ( $jd <= $EDJD[$i])
  ...
  if ($jd != $EDJD[$i]) then
    setenv INFILE1 $INPDIR/MEGANv3.$GDNAM3D.$scen.$MECHANISM.${jd}.ncf
    setenv INFILE2 $INPDIR/MEGANv3.$GDNAM3D.$scen.$MECHANISM.${nd}.ncf
    setenv OUTFILE $OUTDIR/MEGANv3.$GDNAM3D.$scen.rad45.$MECHANISM.${jd}.cst.camx
    setenv RLENG   240000
    setenv TZONE   6
    setenv SDATE   $jd
    setenv STIME   0
