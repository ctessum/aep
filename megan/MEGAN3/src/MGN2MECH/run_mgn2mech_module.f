...
! local variables and their descriptions:
      ...
      REAL, ALLOCATABLE :: inper(:,:,:,:)       ! Input emission buffer (input)
      REAL, ALLOCATABLE :: tmper(:,:,:)       ! Temp emission buffer (temporary)
      REAL, ALLOCATABLE :: outer(:,:,:,:)       ! Output emission buffer (output)
      REAL, ALLOCATABLE :: EF(:,:,:)          ! Emission factor (input)
      REAL, ALLOCATABLE :: GAMNO(:,:,:)         ! NO emission factor (input)
      INTEGER :: n_scon_spc 
      INTEGER :: NVAR,INO
      REAL :: garea 
...
!          CALL CHECKMEM ( ios, 'mech_mwt', PROGNAME )
        ...
        SELECT CASE ( TRIM(MECHANISM) )
          ...
          CASE ('SAPRCII')
            ...
            VNAME3D(1:NVARS3D)     = mech_spc_saprcii(1:NVARS3D)
          CASE ('SAPRC99')
            ...
            VNAME3D(1:NVARS3D)     = mech_spc_saprc99(1:NVARS3D)
          CASE ('RADM2')
            ...
            VNAME3D(1:NVARS3D)     = mech_spc_radm2(1:NVARS3D)
          CASE ('RACM')
            ...
            VNAME3D(1:NVARS3D)     = mech_spc_racm(1:NVARS3D)
          CASE ('CBMZ')
            ...
            VNAME3D(1:NVARS3D)     = mech_spc_cbmz(1:NVARS3D)
          CASE ('SAPRC99X')
            ...
            VNAME3D(1:NVARS3D)     = mech_spc_saprc99_X(1:NVARS3D)
          CASE ('SAPRC99Q')
            ...
            VNAME3D(1:NVARS3D)     = mech_spc_saprc99_Q(1:NVARS3D)
          CASE ('CB05')
            ...
            VNAME3D(1:NVARS3D)     = mech_spc_cb05(1:NVARS3D)
          CASE ('CB6')
            ...
            VNAME3D(1:NVARS3D)     = mech_spc_cb6(1:NVARS3D)
          CASE ('SOAX')
            ...
            VNAME3D(1:NVARS3D)     = mech_spc_soax(1:NVARS3D)
          CASE ('CB6X')
            ...
            VNAME3D(1:NVARS3D)     = mech_spc_cb6x(1:NVARS3D)
          CASE ('RACM2')
            ...
            VNAME3D(1:NVARS3D)     = mech_spc_racm2(1:NVARS3D)
          CASE ('CRIv2')
            ...
            VNAME3D(1:NVARS3D)     = mech_spc_criv2(1:NVARS3D)
          CASE ('SAPRC07')
            ...
            VNAME3D(1:NVARS3D)     = mech_spc_saprc07(1:NVARS3D)
        ENDSELECT
...
!            NVAR = n_cbmz_spc
!          CASE ('SAPRC99X')
!            NVAR = n_saprc99_x_spc
!          CASE ('SAPRC99Q')
!            NVAR = n_saprc99_q_spc
!          CASE ('CB05')
!            NVAR = n_cb05_spc
!          CASE ('CB6')
!            NVAR = n_cb6_spc
...
!...  Loop through time
     ...
     &                      jdate,jtime,inper(T,N,:,:))) THEN
            MESG = 'Error reading '//TRIM(MGN_SPC(nmpmg))
            CALL M3EXIT(PROGNAME,jdate,jtime,MESG,2)
          ENDIF
        ENDDO

        IF ( .NOT. READ3(MGNSEA,'GAMNO',1,jdate,jtime,GAMNO(t,:,:))) THEN
...
!...  Loop through time
      ...
      DO t = 1, mxrec
        ...
        DO s = 1, NVAR
          print*,'VNAME3D(s)=',VNAME3D(s)
          print*,'jdate=',jdate
          print*,'jtime=',jtime
          print*,'outer(t,s,:,:)=',outer(t,s,:,:)
...
!...  End program
!=======================================================================
      END PROGRAM RUN_MGN2MECH_MODULE
