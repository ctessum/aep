...
func (c *ColorMap) Set() {
	...
	if absmax == 0. {
		panic("All the values in the array = 0.")
	}
	...
	if (c.minval-linmin)/(c.minval+linmin) > 0.001 {
		...
		c.stopcolors = append(c.stopcolors, c.cl.interpolate(c.minval/linmax))
	}
	...
	if (c.maxval-linmax)/(c.maxval+linmax) < 0.001 {
		...
		c.stopcolors = append(c.stopcolors, c.cl.interpolate(c.maxval/linmax))
	}
...
func (c *ColorMap) Legend(filename string, label string) (err error) {
	const dpi = 300.
	const aspectRatio = 0.1067
	...
	if c.Font == "" {
		fontConfig = fmt.Sprintf("text-anchor:middle;font-size:%dpx;fill:black",
			int(fontHeight))
	} else {
		fontConfig = fmt.Sprintf(
			"text-anchor:middle;font-size:%dpx;fill:black;font-family:%v",
			int(fontHeight), c.Font)
	}
	width := roundInt(c.LegendWidth * dpi)
	height := roundInt(c.LegendWidth * dpi * aspectRatio)
	...
	g.Rect(barWstart, barHstart, barWidth, barHeight,
		fmt.Sprintf("fill:url(#cmap);stroke:black;stroke-width:%d", strokeWidth))
